 ...
 *
 * Customer aging report with 0-30, 31-60, 61-90, 90+ day buckets
...
import type { CustomerAgingSummary } from '../types/accounts-receivable.types'

export function ARAgingReport() {
  const { t } = useTranslation()
  const { formatAmount } = useCurrency()
  const report = sampleARAgingReport
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
        <AgingCard label={t('accounts.totalOutstanding')} value={report.totalOutstanding} color="border-s-blue-500" />
      ...
      <Card>
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            <CardTitle className="text-base">{t('common.customerAgingDetail')}</CardTitle>
        ...
        <CardContent>
          {report.customers.length === 0 ? (
            <p className="text-center py-8 text-muted-foreground">{t('common.noOutstandingReceivables')}</p>
          ) : (
            <>
              {/* Desktop Table */}
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="bg-muted/50">
                      ...
                      <TableHead className="text-center">{t('common.invoices')}</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {report.customers.map((customer) => (
                      <CustomerAgingRow key={customer.customerId} customer={customer} />
                    ))}
                    ...
                    <CardContent className="pt-3 pb-3">
                      <div className="flex justify-between items-start mb-3">
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground">{customer.invoiceCount} {t('common.invoices')}</p>
                        </div>
                        <p className="font-bold text-lg">{formatAmount(customer.totalOutstanding)}</p>
                      </div>
                      <div className="grid grid-cols-2 gap-2 text-sm">
                        <div className="flex justify-between">
                          <span className="text-green-600">0-30d:</span>
                          <span>{formatAmount(customer.current)}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-amber-600">31-60d:</span>
                          <span>{formatAmount(customer.days31to60)}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-orange-600">61-90d:</span>
                          <span>{formatAmount(customer.days61to90)}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-red-600">90+d:</span>
                          <span>{formatAmount(customer.days90plus)}</span>
                        </div>
                      </div>
                    </CardContent>
                  </Card>
...
function CustomerAgingRow({ customer }: { customer: CustomerAgingSummary }) {
  const { formatAmount } = useCurrency()
  return (
    <TableRow>
      <TableCell>
        <div>
          ...
          <p className="text-xs text-muted-foreground">{formatDate(customer.oldestInvoiceDate)}</p>
        </div>
      </TableCell>
      <TableCell className="text-end text-green-600">
        {customer.current > 0 ? formatAmount(customer.current) : '-'}
      </TableCell>
      <TableCell className="text-end text-amber-600">
        {customer.days31to60 > 0 ? formatAmount(customer.days31to60) : '-'}
      </TableCell>
      <TableCell className="text-end text-orange-600">
        {customer.days61to90 > 0 ? formatAmount(customer.days61to90) : '-'}
      </TableCell>
      <TableCell className="text-end text-red-600">
        {customer.days90plus > 0 ? formatAmount(customer.days90plus) : '-'}
      </TableCell>
