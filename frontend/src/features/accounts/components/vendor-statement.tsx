...
import { sampleAPBills, sampleVendorStatement } from '@/data/accounts.data'

export function VendorStatementView() {
  const { t } = useTranslation()
  const { formatAmount } = useCurrency()
  // Build unique vendors from AP bills
  const vendors = [...new Map(sampleAPBills.map((bill) => [bill.vendorId, { id: bill.vendorId, name: bill.vendorName }])).values()]
  ...
  const statement = selectedVendorId ? sampleVendorStatement : null
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
              ...
              <Select value={selectedVendorId} onValueChange={setSelectedVendorId}>
                <SelectTrigger><SelectValue placeholder={t('common.selectVendor')} /></SelectTrigger>
                <SelectContent>
                  {vendors.map((v) => (
